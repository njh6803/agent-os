...
def create_app(
    *,
    ...
    플러그인 포트는 관리 라우터에 넘어가 `/plugins` 둘이 쓴다. 트레이스 포트는 아직 아무 라우트도
    쓰지 않는다. 그래도 받는 이유는 이 시그니처가 `serve` 의 계약이기 때문이다 — `main.py` 가
    어댑터를 만들어 넘기는 자리이고(ADR 0010), 05·06 이 라우트를 더하며 라우터에 넘기기 시작한다.
    ...
    app.include_router(admin_router(health=Health(status="ok"), plugins=plugins))
