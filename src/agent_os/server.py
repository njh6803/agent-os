...
def create_app(
    *,
    ...
    포트 둘을 지금 아무 라우트도 쓰지 않는다. 그래도 받는 이유는 이 시그니처가 `serve` 의 계약
    이기 때문이다 — `main.py` 가 어댑터를 만들어 넘기는 자리이고(ADR 0010), 04·05·06 이 라우트를
    더하며 이 포트를 라우터에 넘기기 시작한다. 데이터 라우트가 이 티켓에 없는 것은 의도다.
    ...
    app.include_router(admin_router(health=Health(status="ok")))
