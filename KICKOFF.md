...
- `.claude/skills/open-session/SKILL.md`와 `tools/open_session.ps1`. 지시문의 "어디서"가 새 세션이면 그 세션을 연다. 앱 딥링크(`claude://code/new?q=…`. 폴더를 실으면 신뢰 대화상자 뒤 폴더가 떨어진다)로 지시문이 채워진 세션 화면을 열고, 접근성 트리로 보내기와 분할 보기를 누르고, 제목을 브랜치명으로 바꾼다. 첫 줄이 슬래시 명령이면 앱이 그 `/`를 전각 `／`로 바꿔 넣어 스크립트가 보내기 전에 멈추고, 사람이 첫 글자를 고쳐 보낸다. 딥링크 URL은 윈도 명령줄로 넘어가 잘리므로 8000자 안이어야 한다. 데스크톱 앱과 Windows 전용이다. 칩·`claude --bg`·스케줄 실행을 버린 이유는 일지 2026-09-21 open-session.
