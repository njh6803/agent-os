...
2. 상한은 개발자당 시간당 3회이고 롤링 윈도다(공식 요금제 문서, 2026-09-21 확인). `coderabbit --usage`는 청구 주기 누적과 초기화 날짜만 보여주므로 시간당 잔량을 판정할 수 없다. 그 출력으로 실행을 막지 않는다. 실행이 한도 초과로 거절되면 그때 그 사실을 보고하고 종료한다. 재실행은 한도를 하나 더 쓴다.
