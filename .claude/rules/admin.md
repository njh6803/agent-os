...
- **관리는 실행을 일으키지 않고, 관리 라우터는 도구 포트를 받지 않는다.** 조회 하나가 MCP 서버 여럿을 띄우는 길을 시그니처가 먼저 닫는다. `create_app`은 채널을 위해 도구 포트를 받지만 관리 라우터에 넘기지 않는다(ADR 0010의 2026-09-24 이력). 관리 라우트는 409를 내지 않는다 — 재개할 수 없는 상태를 만날 일이 없다.
...
- 매니페스트 응답의 모양은 ADR 0010의 2026-09-23 이력이 원천이고, 2026-09-26 이력(plugin-toggle)이 그것을 개정했다.
