...
- JSONL 목록은 단건 읽기를 재사용하지 않고 파일마다 헤더와 첫 줄과 마지막 줄만 파싱한다. 전부 파싱하면 목록 하나가 모든 실행의 모든 이벤트를 메모리에 올리고 `limit`은 그 뒤에 자르므로 도움이 되지 않는다. 그래서 가운데 줄이 깨진 파일은 목록에서 요약으로 나오고 단건에서 `PluginError`다. 표지의 이유와 단건의 `PluginError` 문구는 경로와 오류 종류만 싣고 트레이스 내용을 되울리지 않는다 — pydantic 검증 문구의 `input_value`가 잘린 줄의 꼬리(도구 결과)를 싣기 때문이다(`jsonl.py`의 `_describe`). 매니페스트 목록은 반대로 단건 읽기를 그대로 부른다 — 파일 하나가 작고, 디렉터리와 `kind`·`name`이 맞는지 보는 검사가 거기 있다.
