...
- 첫 슬라이스의 어댑터: JSONL TraceSink(`traces/<run_id>.jsonl`, 첫 줄 헤더에 `schema_version`), 파일시스템 PluginSource, MCP ToolSource(langchain-mcp-adapters), 시스템 Clock(UTC, uuid4). ChatAnthropic 인스턴스는 `main.py`가 만든다.
