...
- 첫 슬라이스의 어댑터: JSONL TraceStore(`traces/<run_id>.jsonl`, 첫 줄 헤더에 `schema_version`. 한 실행을 쓴 순서대로 읽고 형식 1 파일도 읽는다), 파일시스템 PluginSource, MCP ToolSource(langchain-mcp-adapters), 시스템 Clock(UTC, uuid4). ChatAnthropic 인스턴스는 `adapters/anthropic.py`의 factory로 `main.py`가 만든다. 어댑터는 자기를 스스로 조립하지 않는다.
