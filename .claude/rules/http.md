...
관리와 채널이 함께 따르는 것이다. 원천은 ADR 0010(조립과 에러 봉투)·0011(fail-closed 인증)·0014(채널의 스트림과 실행 전 실패)·0016(공용 층)이다. 관리에만 걸리는 것은 `admin.md`에 있다. 아래는 확정된 결정이고, 코드와 다르면 코드를 고치거나 ADR을 남긴다.
