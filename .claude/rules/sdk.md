...
- 매니페스트는 관리 API의 응답 타입 그대로라 필드를 바꾸면 `openapi.json`도 바뀐다(ADR 0010의 2026-09-23 이력). 그래서 매니페스트 모델의 독스트링은 한 줄이고 논증은 `#` 주석에 두며, 직렬화하면 늘 실리는 필드가 스키마에서도 required가 되도록 `json_schema_serialization_defaults_required`를 켠다.
