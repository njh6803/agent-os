...
- 2026-09-21 공개 전환으로 전체 리뷰가 무료가 되자 사용자 결정으로 CodeRabbit PR 리뷰를 다시 켰다(PR #12). PR 봇은 둘이고 역할 분담은 operations.md. 별 10개 미만은 자동 리뷰가 없어 PR마다 `@coderabbitai review`로 부른다(OSS 플랜, 시간당 1회). CodeRabbit 체크는 요청이 없거나 한도를 넘어도 `pass`라 필수 검사에 넣지 않는다. 다음에 끌 조건: 비공개로 돌아가거나 봇이 초록만 늘릴 때.
