...
- PR을 열거나 병합하면 그 세션의 작업은 끝이다. 다음 작업은 새 세션이 하고, 시작점은 `next-session` 스킬이 내는 지시문이다. 계기는 훅(`tools/hook_pr_next_session.py`)이 넣는다. 그 새 세션은 `open-session` 스킬이 같은 체크아웃에 클릭 없이 연다(딥링크, 워크트리 없음). 사용자가 이어서 지시하면 따른다.
