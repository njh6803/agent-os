...
| 2 | admin-api | `admin/http` `/plugins`, `/traces`, 위젯 설정. `openapi.json` 내보내기 | first-slice | todo |
| 2 | interrupts | 사람 승인 인터럽트, 체크포인터(`thread_id = run_id`), `RunPaused` 이벤트 | first-slice | todo |
| 3 | web-admin | `web/` 워크스페이스, `apps/admin`(Next.js, 아토믹 디자인), `packages/api-client` | admin-api | todo |
