...
| 20 | **닫힌 수 주장을 리뷰가 민다.** `code-review` 명세 축 브리프에 한 줄: 문서·주석·ADR이 수를 세어 닫는 주장("둘뿐", "유일하다", "전부")은 반례를 찾아 민다. "길은 둘"(PR #56), "관대함은 하나"(PR #51), 면제 조항의 폭(그 앞)이 같은 가족 3회차이고 세 번 다 명세 축이 운으로 잡았다 | `.claude/skills/code-review/` | 일지 2026-09-23-04 |
| 21 | **칩은 병합된 것만 가리킨다.** 칩 워크트리는 스폰 시점의 main에서 뜬다. 피처 브랜치 세션이 그 브랜치에만 있는 파일을 가리키는 칩을 띄우면 그 세션은 없는 파일을 찾는다(PR #56, 1회차). 후보: 칩 프롬프트 첫 줄에 "<PR> 병합 뒤 main을 동기화하고 시작" | 칩을 띄우는 자리의 지침 (`CLAUDE.md` 또는 `next-session`) | 일지 2026-09-23-04 |
