...
이 판정자가 보지 않는 곳이 하나 있다. 범위가 `[tool.pyright] include`라서 `plugins/`는 빠진다.
플러그인은 pyright가 아예 보지 않는 코드이고, 거기까지 넓히는 것은 플러그인을 strict로 타입체크
하겠다는 별개의 결정이다. 필요가 증명되면 그때 ADR로 연다.
